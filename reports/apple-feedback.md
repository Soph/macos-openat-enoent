...
Measured with 20 threads racing one name, 200 trials, 4000 calls per row:

| call | result |
| --- | --- |
| `openat(dirfd, "n", O_CREAT)`, file absent | 25% to 75% spurious `ENOENT` |
| `openat(dirfd, "n", O_CREAT\|O_EXCL)`, file absent | correct: one winner, rest `EEXIST` |
| `openat(dirfd, "n", O_CREAT)`, file exists | correct: all callers succeed |
| `openat(dirfd, "n", O_RDONLY)` and `O_RDWR`, file exists | correct |
| `open(fullpath, O_CREAT)`, file absent | correct: all callers succeed |
